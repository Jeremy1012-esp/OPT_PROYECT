...
public class ControllerImplementation implements IController, ActionListener {
   ...
   private void handleInsertPerson() {
        ...
        // Issue #8: Validación de email con regex en el controller
        ...
        // 4. Inyectamos el Email si se ha introducido
        if (!emailText.isEmpty()) {
            p.setEmail(emailText);
        }
        
        // 5. Guardamos y mostramos mensaje de éxito (Issue #5)
        insert(p);
    ...
    public void handleUpdatePerson() {
        if (update != null) {
            // Issue #8: Validación de email en Update
            String emailText = "";
            if (update.getEmail() != null) {
                emailText = update.getEmail().getText().trim();
            }
            if (!emailText.isEmpty()) {
                String emailRegex = "^[a-zA-Z0-9_+&*-]+(?:\\.[a-zA-Z0-9_+&*-]+)*@(?:[a-zA-Z0-9-]+\\.)+[a-zA-Z]{2,7}$";
                if (!emailText.matches(emailRegex)) {
            ...
            update(p);
            // Issue #6: Mensaje de éxito tras actualizar
    ...
    public void handleDeleteAll() {
        ...
        if (answer == 0) {
            deleteAll();
            // Issue #7: Mensaje de confirmación tras borrar todos
    ...
    /**
     * Issue #9: Muestra una ventana dedicada con el total de personas registradas.
     */
